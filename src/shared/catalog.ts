/**
 * Product Catalog - Read-only product definitions
 *
 * In a real system, this would come from a database or API.
 * For demo purposes, we use a hardcoded catalog with generic pizza shop items.
 *
 * IMPORTANT: Prices are stored in cents (integer-only math).
 * The Main process uses this to look up prices - never trust renderer-provided prices.
 */
...
export interface Product {
  ...
  /** Initial stock quantity (-1 means unlimited) */
  initialStock: number;
}
...
/**
 * Product Catalog - Generic pizza shop menu
 */
...
/**
 * Pre-built Map for O(1) SKU lookups
